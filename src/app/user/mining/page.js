  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          >
            ...
            >
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Mining Requirements</h2>
                <p className="text-slate-400 text-sm">Complete the steps below to unlock mining</p>
              </div>
            </motion.div>

            <div className="grid md:grid-cols-2 gap-6 mb-6">
              {/* Membership */}
              <Card className={`p-6 border ${status?.membershipActive ? 'border-green-500/30' : 'border-red-500/30'}`}>
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${status?.membershipActive ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                    {status?.membershipActive ? <CheckCircle className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
                  </div>
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-white">1. Active Membership</h3>
              ...
              {/* Social Verification */}
              <Card className={`p-6 border ${status?.socialApproved ? 'border-green-500/30' : 'border-[#B48811]/30'}`}>
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${status?.socialApproved ? 'bg-green-500/10 text-green-400' : 'bg-[#B48811]/10 text-[#EBD197]'}`}>
                    {status?.socialApproved ? <CheckCircle className="w-5 h-5" /> : <Upload className="w-5 h-5" />}
                  </div>
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-white">2. Social Follow</h3>
                    <p className="text-slate-400 text-sm mt-1">
                      ...
                      <div className="flex flex-wrap gap-3 mt-3">
                        {status.socialLinks.map((link) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 text-[#EBD197] hover:text-[#BB9B49] text-sm"
                          >
            ...
            {!status?.socialApproved && (
              <Card className="p-6 border border-[#B48811]/30">
                <h3 className="text-lg font-semibold text-white mb-4">Upload Follow Screenshot</h3>
                <form onSubmit={handleSubmitSocial} className="space-y-4">
                  <div>
                    <label className="block text-slate-400 text-sm mb-2">Select Platform</label>
                  ...
                  <div>
                    <label className="block text-slate-400 text-sm mb-2">Upload Screenshot</label>
                    <input
                      type="file"
                      accept="image/png, image/jpeg, image/jpg, image/webp"
                      onChange={handleFileChange}
                      className="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#B48811]/20 file:text-[#EBD197] hover:file:bg-[#B48811]/30"
                    />
                    {uploading && (
                    ...
                    {preview && (
                      <div className="mt-3">
                        <img src={preview} alt="Preview" className="max-h-40 rounded-xl border border-slate-700" />
                      </div>
                    )}
                    {/* <input
                      type="text"
                ...
                {verifications.length > 0 && (
                  <div className="mt-6 pt-6 border-t border-slate-700/50">
                    <h4 className="text-white font-medium mb-3">Previous Submissions</h4>
                    <div className="space-y-2">
                      {verifications.map((v) => (
                        <div key={v.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 rounded-lg bg-slate-800/30">
                          <div className="flex-1">
                            <p className="text-white text-sm">{v.platform}</p>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {!content ? (
                <Card className="p-8 text-center md:col-span-2">
                  ...
                  <p className="text-slate-500">No mining card available right now. Please check back later.</p>
                </Card>
              ) : (
                <motion.div
                  initial={{ opacity: 0, scale: 0.9 }}
                  ...
                  whileHover="hover"
                  whileTap={{ scale: 0.98 }}
                  className="group relative bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 hover:border-[#B48811]/40 transition-all duration-300 overflow-hidden md:col-span-2"
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-[#B48811]/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                  <div className="relative">
                    ...
                    <h3 className="font-bold text-lg text-white mb-2">{content.title}</h3>
                    <p className="text-slate-400 text-sm mb-2 line-clamp-3">{content.content?.substring(0, 150)}...</p>
                    {content.content && content.content.length > 150 && (
                      <button
                        onClick={() => setShowReadMore(true)}
                        className="text-[#EBD197] hover:text-[#BB9B49] text-sm mb-4 inline-flex items-center gap-1"
                      >
                        Read More
                      </button>
                    )}
                        ...
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 border rounded-lg flex items-center justify-center ${
                            h.status === 'MISSED'
                              ...
                              : 'bg-gradient-to-br from-[#EBD197] via-[#B48811] to-[#BB9B49] border-[#B48811]/20'
                          }`}>
                            {h.status === 'MISSED'
                              ? <X className="w-5 h-5 text-red-400" />
      ...
      {showReadMore && content && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setShowReadMore(false)}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-slate-700">
              <h3 className="text-xl font-bold text-white">{content.title}</h3>
              <button onClick={() => setShowReadMore(false)} className="text-slate-400 hover:text-white">
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto">
              <p className="text-slate-300 text-sm whitespace-pre-wrap leading-relaxed">{content.content}</p>
            </div>
      ...
      {showMineModal && content && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={closeMineModal}>
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-slate-700">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-[#EBD197] via-[#B48811] to-[#BB9B49] rounded-xl flex items-center justify-center">
                  <FileText className="w-5 h-5 text-white" />
                </div>
                <h3 className="text-xl font-bold text-white">{content.title}</h3>
              </div>
              <button onClick={closeMineModal} className="text-slate-400 hover:text-white">
                <X className="w-6 h-6" />
              </button>
            </div>
            <div className="p-6 overflow-y-auto flex-1">
              ...
              <label className="flex items-start gap-3 cursor-pointer p-3 rounded-xl bg-slate-800/30 border border-slate-700 hover:border-[#B48811]/40 transition-colors">
                <input
                  ...
                  className="w-5 h-5 mt-0.5 accent-[#B48811] cursor-pointer"
                />
            ...
            <div className="p-4 border-t border-slate-700 flex gap-3 justify-end">
              <Button onClick={closeMineModal} variant="outline" className="rounded-full">
                Cancel
              </Button>
              <Button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setShowSuccessModal(false)}>
          <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-8 text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Mining Completed Successfully!</h3>
              <p className="text-slate-400 text-sm mb-4">Your mining reward has been credited to your Widget A wallet.</p>
              {minedReward && (
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-6">
                  <Coins className="w-5 h-5 text-[#EBD197]" />
                  <span className="text-[#EBD197] font-bold text-lg">+{minedReward} Coins</span>
                </div>
              )}
      ...
      {showAlreadyMinedModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={() => setShowAlreadyMinedModal(false)}>
          <div className="bg-slate-900 border border-amber-500/30 rounded-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-8 text-center">
              <div className="w-20 h-20 mx-auto mb-4 bg-amber-500/10 border border-amber-500/30 rounded-full flex items-center justify-center">
                <AlertCircle className="w-10 h-10 text-amber-400" />
              </div>
              <h3 className="text-2xl font-bold text-white mb-2">Already Mined</h3>
              ...
              <div className="flex justify-center">
                <Button onClick={() => setShowAlreadyMinedModal(false)} variant="primary" className="rounded-full">

...
export default function SignInPage({ title = 'Sign In', subtitle = 'Sign in to your EpochEra account', otherPath = null, otherLabel = null }) {
      ...
      <div className="w-full max-w-md bg-card/10 backdrop-blur-xl border-2 border-[#B48811]/50 rounded-3xl p-10 shadow-2xl relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-end">
            <Link href="/forgot-password" className="text-sm text-[#EBD197] hover:text-[#B48811] transition-colors">
              Forgot password?
            </Link>
          </div>
        ...
        <div className="mt-8 text-center space-y-2">
          ...
          {otherPath && otherLabel && (
            <p className="text-muted-foreground text-sm">
              {otherLabel}{' '}
              <Link href={otherPath} className="text-[#EBD197] hover:text-[#B48811] font-medium transition-colors">
                Sign in here
              </Link>
            </p>
          )}
